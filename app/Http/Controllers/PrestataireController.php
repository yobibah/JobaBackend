    ...
    public function prestataire(Request $request)
{
    ...
    $users = Prestataire::inRandomOrder()->get();
    ...
    $usersWithRatings = $users->map(function ($user) {
        $ratingCount = rating::where('prestataires_id', $user->id)->count();
        $ratingSum = rating::where('prestataires_id', $user->id)->sum('notes');
